...
BC = 'periodic'; % current options: 'reflecting', 'open', 'periodic'
...
grad_slope = 0; % modifies the path through the gradient
...
sigma_T = 0.1; % seed dispersal radius savanna trees
sigma_W = 0.025; % fire spread radius
sigma_R = 0.5; % fire spread radius
...
% forest IC
G0 = 0.4*ones(1,N+1) + 0.05*rand(1,N+1);%phi(0.95, 0.05, 0:delta:L, 0.5, s_2);
S0 = 0.1*ones(1,N+1) + 0.05*rand(1,N+1);%0.1*ones(1,N+1);
T0 = 0.3*ones(1,N+1) + 0.05*rand(1,N+1);%phi(0.95, 0.05, 0:delta:L, 0.1, 0.01);%0.1*ones(1,N+1);
F0 = 0.2*ones(1,N+1) + 0.05*rand(1,N+1);
...
for i = 2:n
    ...
    for k = 1:N+1
        ...
        integrand3 = tempR(k,:).*(G(i-1,:)+S(i-1,:)+T(i-1,:));
        temp3(1,k) = sum(integrand3.*Trap)*delta;
    end
    temp1 = temp1/(C_F);
    temp2 = temp2/(C_T);
    temp3 = temp3/(C_R);
    G(i,(N+1):2*N+1) = G(i-1,(N+1):2*N+1) + h*( mu.*S(i-1,(N+1):2*N+1) + nu.*T(i-1,(N+1):2*N+1)...
        - alpha.*temp1.*G(i-1,(N+1):2*N+1).*(1 - 1/r + temp3/r) + ...
        phi(f_0_ref, f_1_ref, E(i-1,:), t_2_ref, s_2).*F(i-1,(N+1):2*N+1)...
        - beta.*temp2.*G(i-1,(N+1):2*N+1) );
    S(i,(N+1):2*N+1) = S(i-1,(N+1):2*N+1) + h*( -mu.*S(i-1,(N+1):2*N+1) + beta.*temp2.*G(i-1,(N+1):2*N+1)...
        - alpha.*temp1.*S(i-1,(N+1):2*N+1).*(1 - 1/r + temp3/r)...
        - omega(w_0_ref, w_1_ref, E(i-1,:), t_1_ref, s_1).*S(i-1,(N+1):2*N+1) );
    T(i,(N+1):2*N+1) = T(i-1,(N+1):2*N+1) + h*( - nu.*T(i-1,(N+1):2*N+1) ...
        + omega(w_0_ref, w_1_ref, E(i-1,:), t_1_ref, s_1).*S(i-1,(N+1):2*N+1)...
        -alpha.*temp1.*T(i-1,(N+1):2*N+1).*(1 - 1/r + temp3/r) );
